...
Gildra Kit Manifest (config/kit.json)
        │
        ├── версии Harness, Node, pnpm, CodeGraph и плагинов
...
команд через RPC намеренно отсутствует: Terminal остаётся инструментом Harness
и использует его permission layer. Нативный host предоставляет только:
