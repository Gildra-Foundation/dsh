...
- **Очереди на лок репозитория нет — есть честный отказ.** Создание
  ветки+worktree сериализовано локом `repo-<projectId>` с таймаутом 10 с.
  Замер: 20 одновременных `createWorkspace` укладываются примерно в 1 с, то
  есть запас около 10×; на машине примерно вдесятеро медленнее последние из
  20 параллельных созданий начнут получать `WORKSPACE_BUSY` вместо ожидания.
  Это осознанный выбор (предсказуемый отказ вместо неограниченной очереди),
  но при росте числа одновременных сессий его стоит держать в виду.
