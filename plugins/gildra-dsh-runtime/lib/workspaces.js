...
export function createWorkspaceManager({ store, roots, projects, leases, processes, journal, env = process.env }) {
  ...
  // Merge workflow вынесен в собственный модуль (Modularity Analyzer нашёл
  // смешение ответственностей); публичный API менеджера не изменился.
  const mergeWorkflow = createMergeWorkflow({ store, roots, projects, journal, withRepoLock, listRecords })
